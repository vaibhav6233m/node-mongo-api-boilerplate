...
const userController = {
  ...
  changePassword: async (req, res) => {
    try {
      const changePasswordDetails = await userObject
        .userService()
        .changePassword(res.locals.tokenInfo, res.locals.requestedData);
      res.send(
        functions.responseGenerator(
  ...
  forgotPassword: async (req, res) => {
    try {
      const forgotPasswordDetails = await userObject
        .userService()
  ...
  resetPassword: async (req, res) => {
    try {
      const resetPasswordDetails = await userObject
        .userService()
  ...
  updateProfile: async (req, res) => {
    try {
      const updateProfileDetails = await userObject
        .userService()
        .updateProfile(res.locals.tokenInfo, res.locals.requestedData);
      res.send(
        functions.responseGenerator(
  ...
  userInformation: async (req, res) => {
    try {
      const userInformationDetails = await userObject
        .userService()
        .userInformation(res.locals.tokenInfo);
      res.send(
        functions.responseGenerator(
  ...
  uploadProfilePicUsingBase64Data: async (req, res) => {
    try {
      const uploadProfilePicDetails = await userObject
        .userService()
        .uploadProfilePicUsingBase64Data(res.locals.tokenInfo.id, res.locals.requestedData);
      res.send(
        functions.responseGenerator(
